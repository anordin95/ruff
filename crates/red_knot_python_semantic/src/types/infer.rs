...
impl<'db> TypeInferenceBuilder<'db> {
    ...
    fn infer_import_from_definition(
        ...
        if let Symbol::Type(ty, re_export, boundness) = module_ty.member(self.db(), name) {
            tracing::debug!(
                "from {module_name} import {name} -> {ty} ({re_export:?})",
        ...
        if let Some(submodule_name) = ModuleName::new(name) {
            ...
            if let Some(submodule_ty) = self.module_type_from_name(&full_submodule_name) {
                let re_export = if asname.as_ref().is_some_and(|asname| asname == name) {
                    ReExport::Explicit
                } else {
                    ReExport::Implicit
                };
